...
def analytics_summary() -> dict[str, Any]:
    ...
    # keep legacy USD balances when no USD transactions were parsed.
    #
    # We allow reasonably large USD balances (your statements often have amounts
    # well above 1,000 USD) while still rejecting obvious artifacts.
    for row in statements:
        if row['id'] not in usd_balance_by_statement and 0 < (row['usd_balance'] or 0) <= 200000:
